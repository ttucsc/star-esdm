function [ ncdf_out, vdata, nc_in, varUnits ] = ncdf_read_file_tll(fname, varName, latrange, lonrange, daterange, out_calendar, do_random, out_varName, is_indexes, closure_flags, maxmem, ok_wrap_lons, do_rotate)
% function to read selected data from a netcdf file.
% This is like ncdf_read_file(...), but returns data in same ordering as
% original file (llt or tll), unless do_rotate is set
...
%           maxmem          optional.  [].  Max memory available to process for reading variables.  s/b less than 1/2 total memory available on the CPU. 
%                                           If maxmem <=1024, it is treated as a # of GB avail;  else treated as # of bytes available. 
%                                           
%           ok_wrap_lons    optional.  [false].  If true, data read can wrap through 360 degrees (e.g.  350 - 10) 
%           do_rotate       optional.  [false]   If true, reverses order of data's dimensions (llt -> tll or tll->llt) 
%
%
%   Returns:
...
%-------------------------------------------------
    ...
    if (~exist('maxmem','var')), maxmem = []; end
    if (~exist("ok_wrap_lons", "var") || isempty(ok_wrap_lons)), ok_wrap_lons = false; end
    if (~exist("do_rotate","var") || isempty(do_rotate)), do_rotate = false; end
    ...
    mem_orig = ncObj.maxmem();
    if (~isempty(maxmem))
        if (maxmem <= 1024)
            ncObj.maxmem(maxmem*1024*1024*1024);
        else
            ncObj.maxmem(maxmem);
        end
    end
    ...
    [latname, lonname, timename, ~, latdimix, londimix,timdimix] = ncdf_get_llt_dimnames(nc_in);
    dimorder_in = [londimix, latdimix, timdimix];
%   dimorder_names([dimorder_in]) = [string([lonname,latname,timename])];

    lats = ncdf_getvar(nc_in, latname, true);
    lons = ncdf_getvar(nc_in, lonname, true);
    lons = mod(lons,360);   % make sure they're 0-360.
    lonrange = mod(lonrange, 360);
    ...
    if (is_indexes)
        ...
        if (~ok_wrap_lons)
            lonrange = max(lons(1), min(lons(end), lonrange));
        end
...
%           [keepers, keepers_ix, dest_ix, outnpts] = find_keepers_in_date_range(daterange, calendar, start_vec, days_since);
    ...
    if (isempty_s(out_calendar)), out_calendar = calendar; end
            % output ncdf
    ncdf_out = make_ncdf(latout, lonout, daterange, calendar, out_varName, out_varName, varUnits, FillValue, [], dimorder_in, do_rotate);
            % create output array
            
            % extract data from the file
    vdata = read_data(nc_in, varName, latix, lonix, keepers, outnpts, dest_ix);

    if (do_rotate), vdata = permute(vdata,[3,2,1]); end
    ...
    ncObj.maxmem(mem_orig);     % reset maxmem to original value.
end

function ncdf_out = make_ncdf(lats, lons, daterange, out_calendar, varName, longvarName, varUnits, FillValue, vdata, dimorder_in, do_rotate)
    ...
    if (all(dimorder_in  == [3,2,1]))
        dimensions = {londim, latdim, timedim};
    else
...
%       [start, count, ixlat, ixlon, ixtime] = ncdf_make_readinfo_var(ncvar, latix, lonix, ix1:ix2);
        [start, count,     ~,     ~, ixtime] = ncdf_make_readinfo_var(ncvar, latix, lonix, ix1:ix2);
...
%               [start, count, ixlat, ixlon, ixtime] = ncdf_make_readinfo_var(ncvar, latix(la), lonix(lo), ix1:ix2);
                [start, count,     ~,     ~, ixtime] = ncdf_make_readinfo_var(ncvar, latix(la), lonix(lo), ix1:ix2);
                indata = ncobj.readvar(varName, start, count,[], ncObj.verbose);
...
%                 vdata(dest_ix,la,lo) = indata(keepers(ix1:ix2));
    ...
    if (dest_ix(1) == 1 && outnpts == (ix2-ix1+1))
        vdata = indata;
    else
