function [clim, clim_1D] = climatology_3d_gauss_V3(data, yrlen, nclim_years, nterms, sig_terms, n_ext_years)
%        clim = climatology_3d_gauss_V2(data, yrlen, nclim_years, nterms, sig_terms, n_ext_years)
%
...
%           yrlen           length of year, in days.  365, usually.
...
%__________________________________________________________________________________________________
    ...
    nyears = length(data)/yrlen;
    if (mod(nyears,1) ~= 0)
        throw(MException('ICSF:BAD_INPUT',sprintf('data length (%d, %f yrs) is not a multiple of year length (%d).', length(data), nyears, yrlen)));
    end
    ...
    y_ext = extend_data(data, yrlen, n_ext_years, nadd_start, nadd_end);     % data, extended so circular convolution of FFT doesn't affect opposite ends of data.

    mpts = length(y_ext);
    myrs = mpts/yrlen;            % this should be 256 (or 512)
    ...
    ix1 = yrlen * nadd_start + 1;
    ix2 = ix1 + (yrlen * nyears) - 1;
    clim_1D = y2(ix1:ix2);
 
            % now, reshape and lpf along the years.
            % See Math Notes above.
    ysurf = reshape(y2, yrlen, myrs);
...
function y_ext = extend_data(y, yrlen, n_ext_years, nadd_start, nadd_end)
...
%
    ...
    ystart=reshape(y(1:clen),yrlen,min_ext_years);                          % reshape 1st years to 365xnstart
    ...
    yend=reshape(y((end-clen+1):end),yrlen,min_ext_years);                  % and do the same for the end few years
