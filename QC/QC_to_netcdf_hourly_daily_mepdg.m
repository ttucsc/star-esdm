function [outTbl] = QC_to_netcdf_hourly_daily_mepdg(varNames, stnInfo, yrRange, varargin)
% Program to convert csv or txt data to netcdf files.
%
%   THIS IS A SPECIAL VERSION FOR ANNE's MEPDG project to replace missing data in HadGEM2 model data.
%       rhsmin and rhsmax HadGEM2-ES files:  2099 data is duplicated into 2100, since that year is missing.
%       tasmin and tasmax HadGEM@-CC files:  Dec. 2004 is duplicated into Dec. 2005, since that month is missing.
%
...
%
%   Needs nearest_ghcn stuff to be fixed, Ian!
%
%
%--------------TO DO----------------
    ...
    for istn=1:istep:nstns            % istep is for for debugging
        for iv=1:nvars
            ...
            vals=jc_units_conversion(vals, Parms.units(iv), Parms.outUnits(iv));
                ...
                % code added to fix NAs in HadGEM2 data
                
            [dup_2099, dup_dec_2004] = parse_filename_for_dups(fnm);
            if (dup_2099)
                vals = dup_2099_data(vals, Parms.yrRange(1), fnm);
            elseif (dup_dec_2004)
                vals = dup_2004_data(vals, Parms.yrRange(1), fnm);
            end
...
function vals = dup_2099_data(vals, start_year, fname)

    fprintf("file %s:  duplication 2099 into 2100\n", fname);
   
    
    ix1 = datenum([2100,1,1]) - datenum(start_year,1,1)+1;
    ix2 = ix1 + 365 - 1;
    ...
    if (nnas ~= 365)
        fprintf("oops:  %d NAs replaced, expecting 365\n", nnas);
    end
            
end

function vals = dup_2004_data(vals, start_year, fname)

    fprintf("file %s:  duplication Dec. 2004 into dec. 2005\n", fname);
   
    ix1 = datenum([2005,12,1]) - datenum(start_year,1,1)+1;
    ix2 = ix1 + 31 - 1;
    ...
    if (nnas ~= 31)
        fprintf("oops:  %d NAs replaced, expecting 31\n", nnas);
    end
...
% timestr is "Daily" or "Hourly"
    ...
    nc = ncdf('', 'Filename',outName, 'Format','netcdf4');      % create an empty ncdf
...
%   nearest_ghcn = (char(workTbl.nearest_ghcn))';
    ...
    stnIDVar = Variable('stnID','Dimensions',[idLen_dim, stn_dim],'Datatype','string','FillValue',' '); 
    ...
    stnNameVar = Variable('stnName','Dimensions',[nmLen_dim, stn_dim],'Datatype','string','FillValue',' ');
    ...
    varNameVar.putatt('description',sprintf('Variable Names for data varaibles, stored as fixed length char strings %d chars long, padded with trailing spaces',varNameLen));
    ...
    nearest_ghcnVar = Variable('nearest_ghcn','Dimensions', [ghcnLen_dim, stn_dim],'Datatype','string','FillValue',' ');
...
function [dup_2099, dup_dec_2004] = parse_filename_for_dups(fname)

    if (~isstring(fname)), fname = string(fname); end
    ...
    dup_2099 = false;
    dup_dec_2004 = false;
    if (strcmp(model,"HadGEM2-ES") && any(strcmp(varnm,["rhsmin","rhsmax"])))
        dup_2099 = true;
    elseif (strcmp(model,"HadGEM2-CC") && any(strcmp(varnm,["tasmin","tasmax"])))
        dup_dec_2004 = true;
    end
end        
...
function   workTbl = add_QC_columns(stnInfo,          RP)
    %  dupflags: table w/ 2 columns:  dup_2099 and dup_dec_2004 for fixing problems with HadGEM-CC tasmin/max and
    %  HadGEM-ES rhsmin/max stuff.
...
% yrRange, ncComments, toDegF, toMM, basedir, fnameCol
    ...
    if (isempty(Parms.ghcn_stninfo))
        load("/Volumes/jcsf_data/data/obs/stations_netcdf/stations.ncamerica.tmax.mat",'tbl');  
