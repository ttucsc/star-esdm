function varargout = plot_google_map_ic(varargin)
...
%   verbose           (logical, false).  If true, outputs google API call URL.
%
% OUTPUT:
...
%  Zohar Bar-Yehuda
%
%       cleaned up icsf 3/7/2017 to get rid of most of the compiler warnings...
%       modified icsf 6/7/2019 to fix obsolete matlab code
%
%   TODO:
%       1.  replace parameter setup with matlab parser
%       2.  add a timer to avoid constant calls to redraw map.
%               should start/reset a timer on every resize, and only call for new google map after .5 seconds idle
%               should recalculate area to display after resize operation.
%       3.  Merge with draw_google_map, and draw boundary/shape via google, rather than via separate plotting
%
    ...
    for idx = 1:2:length(varargin)
        switch lower(varargin{idx})
            ...
            case 'apikey'
                ...
                % save key to file
                funcFile = which('plot_google_map_ic.m');
...
for idx = 1:length(markerlist)
    if (isempty(markerlist{idx}))
        continue;
    elseif idx < length(markerlist)
        markers = sprintf('%s%s%s',markers, markerlist{idx},'%7C');
    else
        markers = sprintf('%s%s',markers, markerlist{idx});
    end
end
...
catch my_err % error downloading map
...
%       set(figHandle, 'ResizeFcn', @update_google_map_fig);       
        set(figHandle, 'SizeChangedFcn', @update_google_map_fig);       
    end    
...
if isfield(ud, 'gmap_params')
    ...
    plot_google_map_ic(params{:});
end


% function update_google_map_fig(obj,evd)
function update_google_map_fig(~,~)
% callback function for auto-refresh
drawnow;
...
for idx = 1:length(axes_objs)
    if ~isempty(findobj(get(axes_objs(idx),'children'),'tag','gmap'))
        ...
        if ~sum(strcmpi(params, 'Axis'))
            params = [params, {'Axis', axes_objs(idx)}]; %#ok<AGROW>
        end
        plot_google_map_ic(params{:});
    end
end
